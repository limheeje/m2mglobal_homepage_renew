...
**라우트 맵 (`config/route/index.ts`, 단일 파일)**: GNB 2Depth(`Gnb.astro`)·전체메뉴(`AllMenu.astro`)·서브페이지 2Depth 탭(`Navigation.astro`)·개발용 전체 페이지 목록(`page-list.astro`)이 전부 이 파일의 `route: Route[]` 배열 하나를 소스로 쓴다 — `import {route} from '@config/route'`. 각 그룹(`{label, href, children}`)의 `children`(`{id, koLabel, label, href}`)이 실제 메뉴 항목이고, 링크는 `` `${PUBLIC_BUILD_URL}${item.href}` `` 문자열 접합으로 만든다(기존 호출부 전부 이 방식 — 미리 계산된 URL을 조회하는 별도 헬퍼는 없음). 각 항목 옆에는 `@old/header.html`(구 GNB) 대비 신규 구조가 어떻게 바뀌었는지(existing/renamed/new/merged) 주석으로 기록되어 있다(런타임 미사용, 이력 추적용). 새 GNB 메뉴가 추가되면 이 배열에 항목을 추가하기만 하면 GNB/전체메뉴/서브탭/page-list에 전부 자동 반영된다. ⚠️ **`route.find(item => item.href === ...)`는 항상 그 메뉴 그룹의 최상위 href(보통 그룹 첫 번째 child의 href와 동일)로 조회할 것** — 페이지 자기 자신의 href를 넣으면 `Navigation`이 빈 값을 받아 2Depth 서브 내비게이션이 사라진다(과거 9개 페이지에서 동시 발생했던 버그, 아래 작업 로그 참고). (이전엔 이 파일이 `index.ts`와 `routeMap.ts` 두 개로 나뉘어 있었고 `index.ts` 쪽 링크 조회 헬퍼(`route: Record<string,string>`)는 어디서도 안 쓰이는 죽은 코드였다 — 2026-08-20에 하나로 통합, git 이력에서 이전 구조 확인 가능.)
...
### 2026-08-20
- `/404` 페이지 신규 추가(`src/pages/404.astro` + `src/styles/scss/page/_error.scss`) — Astro 예약 경로, Figma 대응 디자인 없는 유틸리티 페이지라 `TitleMain`+`Button`만 조합. 자동 리다이렉트 없이 "홈으로 가기" 버튼만 제공(사용자 확인 후 결정). `base`(`PUBLIC_BUILD_URL`) 서브패스 유무와 무관하게 `dist/404.html`은 항상 같은 위치에 생성되지만, 실제 404 응답 시 이 파일을 보여줄지는 호스팅 서버(Nginx/Apache/IIS 등)의 오류 페이지 설정 몫 — Astro 밖의 인프라 영역이라 별도 확인 필요.
- **리팩터링**: `config/route/`가 `index.ts`(링크 조회용 `route: Record<string,string>` — 실제로는 어디서도 import되지 않던 죽은 코드)와 `routeMap.ts`(GNB/AllMenu/Navigation/page-list가 실제로 쓰던 `route: Route[]`)로 나뉘어 있던 걸 `config/route/index.ts` 한 파일로 통합. `routeMap.ts` 삭제, 24개 파일의 import를 `@config/route/routeMap.ts` → `@config/route`로 일괄 변경. `RouteEntry`(status/oldPath/note 기반 구조)의 Old→New 마이그레이션 이력은 런타임 데이터가 아니라 각 라우트 항목 옆 주석으로 이동. `astro check`로 0 errors 확인.
